#ifndef HEADERS_FUNCTIONS_OVERLAY00_H_
#define HEADERS_FUNCTIONS_OVERLAY00_H_
struct c_field_scene * CFieldSceneConstructor(struct c_field_scene *this,struct megastruct *param_2);
struct c_field_scene * struct CFieldSceneBaseDestructor(struct c_field_scene *param_1);
struct c_field_scene * struct CFieldSceneDeletingDestructor(struct c_field_scene *param_1);
void ret_0211e958(void);
void AddEnemyFormToEncounterType2(struct c_field_scene *param_1,unsigned int param_2,int param_3,unsigned int param_4);
void Overlay00PreEntryPoint(undefined4 param_1,struct megastruct *param_2,undefined4 param_3,undefined4 param_4);
void OpenAndInitMapScriptFile(struct unk_struct_size_0x80 *param_1,int param_2);
void ret_02125334(void);
undefined4 MaybeOpenAndInitMapDatAndTexFiles(struct unk_struct_size_0x80 *param_1,undefined4 param_2,char *param_3,int param_4);
void ret_021266ac(void);
void ret_021266b0(void);
void ret_021266c4(void);
void ret_021266c8(void);
void ret_0213524c(void);
undefined4 MaybeOpenAndInitTargetClearScript(void *param_1,int param_2,int param_3);
void MaybeOpenAndInitTargetObjectTouchScript(undefined2 *param_1,char *param_2,undefined2 param_3);
void ret_0214234c(void);
void ret_02142398(void);
void ret_021427ec(void);
void ret_02142ab0(void);
void ret_0214340c(void);
void ret_02143ae0(void);
void ret_02144368(void);
void ret_0214436c(void);
void ret_02145108(void);
void ret_02145134(void);
void ret_02145138(void);
void ret_02145804(void);
void ret_02145b4c(void);
void ret_02145b50(void);
void ret_02146168(void);
void ret_021464bc(void);
void ret_02146698(void);
void ret_02146a18(void);
void ret_02146b20(void);
void ret_02146f60(void);
void ret_02146f64(void);
void ret_02147564(void);
void ret_02147634(void);
void ret_02147a74(void);
void ret_02147a78(void);
void ret_02147ffc(void);
void ret_021480cc(void);
void ret_021485c0(void);
void ret_021485c4(void);
void SetSysCallPage10(void);
undefined4 SysCallReturnZero1000(void);
void ret_02149118(void);
void ret_0214911c(void);
void SetSysCallPage11(void);
undefined4 SysCallReturnZero1100(void);
void ret_02149984(void);
void ret_02149988(void);
void SetSysCallPage12(void);
undefined4 SysCallReturnZero1200(void);
void ret_02149a74(void);
void ret_02149a78(void);
void SetSysCallPage13(void);
undefined4 SysCallReturnZero1300(void);
void ret_02149e58(void);
void ret_02149e5c(void);
void SetSysCallPage14(void);
undefined4 SysCallReturnZero1400(void);
undefined4 SysCallMaybeCallsScriptFunctionByName(undefined4 *param_1,int param_2);
undefined4 SysCallInitCombatType2(undefined4 *param_1,int param_2);
undefined4 TryMountPokemon(struct pokemon_data *param_1);
unsigned int SysCallRetrievePokemonByUniqueId(int *param_1);
struct enum room_id_16 SysCallGetFlyDestinationRoom(void);
int16_t SysCallGetFlyXCoord(void);
int16_t SysCallGetFlyYCoord(void);
void ret_0214b6f0(void);
void ret_0214b6f4(void);
void SetSysCallPage16(void);
undefined4 SysCallReturnZero1600(void);
void ret_0214b93c(void);
void ret_0214b940(void);
void SetSysCallPage15(void);
undefined4 SysCallReturnZero1500(void);
undefined4 SysCallReturnZero1501(void);
undefined4 SysCallReturnZero1502(void);
undefined4 SysCallReturnZero1503(void);
undefined4 SysCallReturnZero1504(void);
int8_t GetStylerLevel(void);
bool GetPartner1Status(void);
void SetPartner1Status(bool param_1);
bool GetPartner2Status(void);
void SetPartner2Status(bool param_1);
bool GetPartner3Status(void);
void SetPartner3Status(bool param_1);
bool GetPartner4Status(void);
void SetPartner4Status(bool param_1);
bool GetPartner5Status(void);
void SetPartner5Status(bool param_1);
bool GetPartner6Status(void);
void SetPartner6Status(bool param_1);
bool GetPartner7Status(void);
void SetPartner7Status(bool param_1);
bool GetPartner8Status(void);
void SetPartner8Status(bool param_1);
bool GetPartner9Status(void);
void SetPartner9Status(bool param_1);
bool GetPartner10Status(void);
void SetPartner10Status(bool param_1);
bool GetPartner11Status(void);
void SetPartner11Status(bool param_1);
bool GetPartner12Status(void);
void SetPartner12Status(bool param_1);
bool GetPartner13Status(void);
void SetPartner13Status(bool param_1);
bool GetPartner14Status(void);
void SetPartner14Status(bool param_1);
bool GetPartner15Status(void);
void SetPartner15Status(bool param_1);
bool GetPartner16Status(void);
void SetPartner16Status(bool param_1);
bool GetPartner17Status(void);
void SetPartner17Status(bool param_1);
int8_t GetActiveQuestId(void);
int8_t GetActiveMissionId(void);
undefined* DebugGrabFileNameAndMsgId(void);
void DebugPrintAllScriptVars(void);
undefined *DebugPrintPartnerPokemonPokeAssists(int param_1,undefined4 param_2,undefined4 param_3,undefined4 param_4);
int64_t GetCaptureLineLenRecordSimple(void);
void SetCaptureLineLenRecordSimple(int param_1);
int32_t GetNumLoopsRecordSimple(void);
void SetNumLoopsRecordSimple(undefined4 param_1);
int16_t GetPokemonCapturedRecordSimple(void);
void SetPokemonCapturedRecordSimple(undefined2 param_1);
int16_t GetPokemonFledRecordSimple(void);
void SetPokemonFledRecordSimple(undefined2 param_1);
int16_t GetPokeAssistsUsedRecordSimple(void);
void SetPokeAssistsUsedSimple(undefined2 param_1);
int GetMaxPlayTimeSeconds(void);
int32_t GetCurrentPartnerPokemonPokeAssists(void);
void ret_0215061c(void);
void ret_02150620(void);
void ret_02150880(void);
void ret_02151818(void);
void ret_0215181c(void);
void ret_02151820(void);
void ret_02151824(void);
void ret_02151828(void);
void ret_0215182c(void);
void ret_02151830(void);
void ret_02151834(void);
void ret_02151838(void);
void ret_0215183c(void);
void ret_02151840(void);
void ret_02151844(void);
void ret_02151898(void);
void ret_0215189c(void);
void ret_021518a0(void);
void ret_021518a4(void);
void ret_021518a8(void);
void ret_0215215c(void);
void ret_02152ab8(void);
void ret_0215ad00(void);
void ret_0215b7fc(void);
void ret_0215bdb4(void);
void ret_0215be88(void);
void ret_0215be8c(void);
void ret_0215d108(void);
void ret_0215d10c(void);
void ret_0215f854(void);
void ret_0215f858(void);
void ret_021603a8(void);
void ret_021603ac(void);
int MaybeOpenAndInitEffectScriptFile(int param_1,int param_2);
void ret_02163cf4(void);
void ret_02163cf8(void);
void ret_02165868(void);
void ret_02165eac(void);
void ret_02165eb0(void);
void ret_02167984(void);
void ret_02167988(void);
void ret_0216798c(void);
void ret_02167990(void);
void ret_0216a5d0(void);
void ret_0216a5d4(void);
void ret_0216ae74(void);
void ret_0216b52c(void);
void ret_0216b530(void);
void ret_0216cb50(void);
void ret_0216cb68(void);
void ret_0216cb80(void);
void ret_0216cc48(void);
void ret_0216cc4c(void);
void ret_0216ebb4(void);
void ret_0216ebb8(void);
void ret_0216f640(void);
void ret_0216f644(void);
void ret_02171b18(void);
#endif